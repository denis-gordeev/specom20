...
"""Finetuning the library models for sequence classification on panic in social media
                ...
                if (
                    ...
                    if (
                        ...
                        try:
                            label_list = [1, 0]
                            results = evaluate(
                                args, model, tokenizer, label_list
                            )
...
def evaluate(args, model, tokenizer, label_list, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            try:
                ...
                with torch.no_grad():
                    ...
                    outputs = model(**inputs)
                    tmp_eval_loss, logits_a = outputs[:3]

                    eval_loss += tmp_eval_loss.mean().item()
                nb_eval_steps += 1
                if preds is None:
                    preds = logits_a.detach().cpu().numpy()
                    out_label_ids_a = inputs["labels"].detach().cpu().numpy()
                else:
                    preds = np.append(
                        preds, logits_a.detach().cpu().numpy(), axis=0
                    )
        ...
        try:
            ...
            if args.do_eval:
                result = compute_metrics(eval_task, preds, out_label_ids_a)
...
def main():
    ...
    if (args.do_predict or args.do_eval) and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            try:
                result = evaluate(
                    args, model, tokenizer, label_list, prefix=prefix
                )
                if args.do_eval:
                    result = dict(
